    ...
    pub fn read_arm7<T: Convert>(&self, addr_offset: u32) -> T {
        ...
        while (index - 3) < mem::size_of::<T>() {
            ...
            io_ports_read!(match addr_offset + (index - 3) as u32 {
                io16(0x4) => self.gpu_context.get_disp_stat::<{ CpuType::ARM7 }>(),
                ...
                io16(0x180) => self.ipc_handler.borrow().get_sync_reg::<{ CpuType::ARM7 }>(),
                io16(0x184) => self.ipc_handler.borrow().get_fifo_cnt::<{ CpuType::ARM7 }>(),
                io16(0x1A0) => self.cartridge_context.borrow().get_aux_spi_cnt::<{ CpuType::ARM7 }>(),
                io8(0x1A2) => self.cartridge_context.borrow().get_aux_spi_data::<{ CpuType::ARM7 }>(),
                io32(0x1A4) => self.cartridge_context.borrow().get_rom_ctrl::<{ CpuType::ARM7 }>(),
                io16(0x1C0) => self.spi_context.borrow().cnt,
                io8(0x1C2) => self.spi_context.borrow().data,
                ...
                io32(0x410) => self.spu_context.borrow().get_cnt(1),
                io32(0x420) => self.spu_context.borrow().get_cnt(2),
                io32(0x430) => self.spu_context.borrow().get_cnt(3),
                io32(0x440) => self.spu_context.borrow().get_cnt(4),
                io32(0x450) => self.spu_context.borrow().get_cnt(5),
                io32(0x460) => self.spu_context.borrow().get_cnt(6),
                io32(0x470) => self.spu_context.borrow().get_cnt(7),
                io32(0x480) => self.spu_context.borrow().get_cnt(8),
                io32(0x490) => self.spu_context.borrow().get_cnt(9),
                io32(0x4A0) => self.spu_context.borrow().get_cnt(10),
                io32(0x4B0) => self.spu_context.borrow().get_cnt(11),
                io32(0x4C0) => self.spu_context.borrow().get_cnt(12),
                io32(0x4D0) => self.spu_context.borrow().get_cnt(13),
                io32(0x4E0) => self.spu_context.borrow().get_cnt(14),
                io32(0x4F0) => self.spu_context.borrow().get_cnt(15),
                io16(0x500) => self.spu_context.borrow().main_sound_cnt,
                io16(0x504) => todo!(),
                io8(0x508) => self.spu_context.borrow().get_cap_cnt(0),
                io8(0x509) => self.spu_context.borrow().get_cap_cnt(1),
                io32(0x510) => todo!(),
                io32(0x518) => todo!(),
    ...
    pub fn write_arm7<T: Convert>(&self, addr_offset: u32, value: T) {
        ...
        while (index - 3) < mem::size_of::<T>() {
            ...
            io_ports_write!(match addr_offset + (index - 3) as u32 {
                ...
                io16(0x180) => self.ipc_handler.borrow_mut().set_sync_reg::<{ CpuType::ARM7 }>(mask, value),
                io16(0x184) => self.ipc_handler.borrow_mut().set_fifo_cnt::<{ CpuType::ARM7 }>(mask, value),
                io32(0x188) => self.ipc_handler.borrow_mut().fifo_send::<{ CpuType::ARM7 }>(mask, value),
                io16(0x1A0) => self.cartridge_context.borrow_mut().set_aux_spi_cnt::<{ CpuType::ARM7 }>(mask, value),
                io8(0x1A2) => self.cartridge_context.borrow_mut().set_aux_spi_data::<{ CpuType::ARM7 }>(value),
                io32(0x1A4) => self.cartridge_context.borrow_mut().set_rom_ctrl::<{ CpuType::ARM7 }>(mask, value),
                io32(0x1A8) => self.cartridge_context.borrow().set_bus_cmd_out_l::<{ CpuType::ARM7 }>(mask, value),
                io32(0x1AC) => self.cartridge_context.borrow().set_bus_cmd_out_h::<{ CpuType::ARM7 }>(mask, value),
                io16(0x1C0) => self.spi_context.borrow_mut().set_cnt(mask, value),
                io8(0x1C2) => self.spi_context.borrow_mut().set_data(value),

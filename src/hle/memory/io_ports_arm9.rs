    ...
    pub fn read_arm9<T: Convert>(&self, addr_offset: u32) -> T {
        ...
        while (index - 3) < mem::size_of::<T>() {
            ...
            io_ports_read!(match addr_offset + (index - 3) as u32 {
                ...
                io32(0xE0) => self.dma.read().unwrap().get_fill(0),
                io32(0xE4) => self.dma.read().unwrap().get_fill(1),
                io32(0xE8) => self.dma.read().unwrap().get_fill(2),
                io32(0xEC) => self.dma.read().unwrap().get_fill(3),
                io16(0x100) => todo!(),
                io16(0x102) => todo!(),
                io16(0x104) => todo!(),
                io16(0x106) => todo!(),
                ...
                io32(0x69C) => todo!(),
                io32(0x6A0) => todo!(),
                io32(0x1000) => self.gpu_2d_context_b.borrow().disp_cnt,
                io16(0x1008) => self.gpu_2d_context_b.borrow().bg_cnt[0],
                io16(0x100A) => self.gpu_2d_context_b.borrow().bg_cnt[1],
                io16(0x100C) => self.gpu_2d_context_b.borrow().bg_cnt[2],
                io16(0x100E) => self.gpu_2d_context_b.borrow().bg_cnt[3],
                io16(0x1048) => todo!(),
                io16(0x104A) => todo!(),
                io16(0x1050) => todo!(),
                io16(0x1052) => todo!(),
    ...
    pub fn write_arm9<T: Convert>(&self, addr_offset: u32, value: T) {
        ...
        while (index - 3) < bytes.len() {
            ...
            io_ports_write!(match addr_offset + (index - 3) as u32 {
                ...
                io32(0x64) => todo!(),
                io16(0x6C) => todo!(),
                io32(0xB0) => self.dma.write().unwrap().set_sad(0, mask, value),
                io32(0xB4) => self.dma.write().unwrap().set_dad(0, mask, value),
                io32(0xB8) => self.dma.write().unwrap().set_cnt(0, mask, value),
                io32(0xBC) => self.dma.write().unwrap().set_sad(1, mask, value),
                io32(0xC0) => self.dma.write().unwrap().set_dad(1, mask, value),
                io32(0xC4) => self.dma.write().unwrap().set_cnt(1, mask, value),
                io32(0xC8) => self.dma.write().unwrap().set_sad(2, mask, value),
                io32(0xCC) => self.dma.write().unwrap().set_dad(2, mask, value),
                io32(0xD0) => self.dma.write().unwrap().set_cnt(2, mask, value),
                ...
                io32(0xE0) => self.dma.write().unwrap().set_fill(0, mask, value),
                io32(0xE4) => self.dma.write().unwrap().set_fill(1, mask, value),
                io32(0xE8) => self.dma.write().unwrap().set_fill(2, mask, value),
                io32(0xEC) => self.dma.write().unwrap().set_fill(3, mask, value),

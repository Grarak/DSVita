...
static mut DELEGATE_FUN: *const fn(usize, usize) = ptr::null();
...
unsafe extern "C" fn sigsegv_handler(sig: i32, si: *mut siginfo_t, segfault_ctx: *mut c_void) {
    ...
    let context = &(*context).uc_mcontext;

    let delegate_fun: fn(usize, usize) = mem::transmute(DELEGATE_FUN);
    delegate_fun(si_addr as usize, context.arm_pc as usize);
...
pub unsafe fn register_abort_handler(delegate: fn(usize, usize)) -> io::Result<()> {

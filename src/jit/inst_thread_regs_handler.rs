...
pub unsafe extern "C" fn restore_thumb_after_restore_spsr<const CPU: CpuType>(emu: *mut Emu) {
    (*emu).thread_restore_thumb_mode(CPU);
}

pub unsafe extern "C" fn set_pc_arm_mode<const CPU: CpuType>(emu: *mut Emu) {
    (*emu).thread_force_pc_arm_mode(CPU)
}

pub unsafe extern "C" fn set_pc_thumb_mode<const CPU: CpuType>(emu: *mut Emu) {
    (*emu).thread_force_pc_thumb_mode(CPU)
}

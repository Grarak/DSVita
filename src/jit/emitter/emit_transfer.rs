use crate::hle::hle::{get_mmu, get_regs};
    ...
    pub fn emit_single_transfer<const THUMB: bool, const WRITE: bool>(
        ...
        let after_host_restore = |asm: &Self, opcodes: &mut Vec<u32>| {
            ...
            opcodes.extend(AluImm::mov32(
                Reg::R1,
                get_regs!(asm.hle, CPU).get_reg(op0) as *const _ as _,
            ));
            if WRITE && op0 == Reg::PC {
            ...
            if write_back && (WRITE || op0 != *og_op1) {
                opcodes.extend(get_regs!(asm.hle, CPU).emit_set_reg(*og_op1, addr_reg, Reg::R2));
            }
        };
    ...
    pub fn emit_single_read_transfer<const THUMB: bool, const WRITE_BACK: bool, const PRE: bool>(
        ...
        let mut slow_read_opcodes = Vec::new();
        {
            ...
            slow_read_opcodes.extend(self.emit_call_host_func(
                |asm, opcodes| {
                    if PRE {
                        ...
                        if WRITE_BACK && op0 != *og_op1 {
                            opcodes.extend(get_regs!(asm.hle, CPU).emit_set_reg(
                                *og_op1,
                                Reg::R0,
                                Reg::R1,
                            ));
                        ...
                        if og_op1.is_emulated() || *og_op1 == Reg::SP {
                            if *og_op1 == Reg::PC {
                                ...
                                opcodes
                                    .extend(get_regs!(asm.hle, CPU).emit_get_reg(Reg::R0, *og_op1));
                            }
                ...
                &[
                    None,
                    Some(get_regs!(self.hle, CPU).get_reg(op0) as *const _ as _),
                    Some(pc),
                    Some(jit_asm_addr),
                ],
                func_addr,
        ...
        let mut mmu_read_opcodes = Vec::new();
        {
            ...
            if op0.is_emulated() {
                mmu_read_opcodes.extend(get_regs!(self.hle, CPU).emit_set_reg(
                    op0,
                    rd,
                    physical_addr_reg,
                ));

...
impl VramMaps {
    ...
    pub fn is_tex_rear_plane_img_dirty(&self, start: u32, end: u32, other: &[u8; 4], dirty_sections: &Bitset<6>) -> bool {
        ...
        let end = utils::align_up(end as usize, BANK_SECTION_SHIFT) as u32;
        for addr in (start..end).step_by(BANK_SECTION_SIZE) {
            let bank_num = addr >> 17;
            let bank_map = self.tex_rear_plane_img[bank_num as usize];
            if !bank_map.is_null() {
    ...
    pub fn is_tex_palette_dirty(&self, start: u32, end: u32, other: &[u8; 6], dirty_sections: &Bitset<6>) -> bool {
        ...
        let end = utils::align_up(end as usize, BANK_SECTION_SHIFT) as u32;
        for addr in (start..end).step_by(BANK_SECTION_SIZE) {
            let bank_num = addr >> 14;
            let bank_map = self.tex_palette[bank_num as usize];
            if !bank_map.is_null() {

...
impl Texture3D {
    ...
    unsafe fn decode_pal256(&mut self, mem_refs: &GpuMemRefs) {
        ...
        for i in (0..self.metadata.size()).step_by(16) {
            ...
            for j in 0..16 {
                let pal_index = utils::read_from_mem::<u8>(mem_refs.tex_rear_plane_image.as_ref(), vram_addr + j as u32) as u32;
                if self.metadata.color_0_transparent() && pal_index == 0 {
                    colors[j] = 0;
                } else {
                    colors[j] = utils::read_from_mem(mem_refs.tex_pal.as_ref(), pal_addr + (pal_index << 1));
                }
            }
    ...
    fn is_dirty(&self, mem_buf: &GpuMemBuf) -> bool {
        ...
        if mem_buf
            .vram
            .maps
            .is_tex_rear_plane_img_dirty(vram_addr, vram_addr_end, &self.tex_rear_plane_img_banks, &mem_buf.vram_banks_dirty_sections_accumulated)
        {
            return true;
        }
        ...
        if self.metadata.format() == TextureFormat::Texel4x4Compressed {
            ...
            if mem_buf
                .vram
                .maps
                .is_tex_rear_plane_img_dirty(slot1_addr, slot1_addr_end, &self.tex_rear_plane_img_banks, &mem_buf.vram_banks_dirty_sections_accumulated)
            {
                return true;
            }
        }
        ...
        if self.metadata.format() != TextureFormat::Direct {
            ...
            if mem_buf
                .vram
                .maps
                .is_tex_palette_dirty(pal_addr, pal_addr_end, &self.tex_palette_banks, &mem_buf.vram_banks_dirty_sections_accumulated)
            {
                return true;
            }
        }
...
impl Texture3DCache {
    ...
    pub fn get(&mut self, polygon: &Gpu3DPolygon, mem_buf: &GpuMemBuf, mem_refs: &GpuMemRefs) -> &mut Texture3D {
        let key = polygon.key();
        if let Some(texture_3d) = self.cache.get_mut(&key) {
            if texture_3d.in_use || !texture_3d.is_dirty(mem_buf) {
        ...
        while self.total_size + texture_3d.metadata.size() >= CACHE_SIZE_LIMIT {
            ...
            for (&key, texture_3d) in &self.cache {
                if !texture_3d.in_use && texture_3d.last_used < oldest_timestamp {
                    oldest_key = key;
